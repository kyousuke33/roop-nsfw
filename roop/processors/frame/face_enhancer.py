...
# Biến toàn cục cho model tăng cường khuôn mặt và các semaphore, lock để đảm bảo an toàn khi chạy đa luồng
...
def get_face_enhancer() -> Any:
    ...
    Nếu chưa có, sẽ tải model từ đường dẫn được chỉ định.
    """
...
def get_device() -> str:
    """
    Xác định thiết bị xử lý: nếu có GPU thì trả về 'cuda'; nếu có CoreML (macOS MPS) thì 'mps'; ngược lại trả về 'cpu'.
    """
...
def clear_face_enhancer() -> None:
    """
    Xóa model face enhancer khỏi bộ nhớ.
    """
    global FACE_ENHANCER
    FACE_ENHANCER = None
...
def enhance_face(target_face: Face, temp_frame: Frame) -> Frame:
    ...
    target_face: Một dictionary chứa thông tin khuôn mặt, bao gồm khóa 'bbox' (hộp giới hạn).
    temp_frame: Frame chứa khuôn mặt cần được tăng cường.
    """
    # Lấy tọa độ của khuôn mặt và tính thêm phần đệm
    ...
    # Cắt khu vực khuôn mặt với đệm
    temp_face = temp_frame[start_y:end_y, start_x:end_x]
    if temp_face.size:
        with THREAD_SEMAPHORE:
            # GFPGANer trả về tuple; chúng ta chỉ lấy phần kết quả đã paste back
            _, _, temp_face = get_face_enhancer().enhance(temp_face, paste_back=True)
        # Ghi kết quả tăng cường trở lại frame gốc
        temp_frame[start_y:end_y, start_x:end_x] = temp_face
    return temp_frame

def process_frame(source_face: Face, reference_face: Face, temp_frame: Frame) -> Frame:
    """
    Xử lý một frame: áp dụng tăng cường cho tất cả các khuôn mặt trong frame.
    """
...
def process_frames(source_path: str, temp_frame_paths: List[str], update: Callable[[], None]) -> None:
    ...
    Xử lý hàng loạt frame cho video:
      - Đối với mỗi frame, áp dụng tăng cường khuôn mặt.
      - Ghi đè kết quả vào file gốc.
      - Gọi hàm update sau khi xử lý mỗi frame nếu có.
    """
...
def process_video(source_path: str, temp_frame_paths: List[str]) -> None:
    """
    Xử lý video bằng cách gọi hàm xử lý hàng loạt frame.
    """
...
def resume_processing_video(source_path: str, video_path: str) -> None:
    ...
    # Lấy danh sách file theo định dạng 4 chữ số (có thể có hoặc không có hậu tố _swapped)
    ...
    # Xác định các chỉ số đã được xử lý (các file có hậu tố '_swapped')
    ...
    # Tạo danh sách các file chưa được swap (chỉ nhận file có tên 4 chữ số không chứa '_swapped')
    files_to_process = []
    for file in all_files:
        base = os.path.basename(file)
        if "_swapped" in base:
    ...
    if not files_to_process:
        ...
        for idx, file in files_to_process:
            ...
            img = cv2.imread(file)
            # Với face_enhancer, ta không cần khuôn mặt nguồn nên truyền None
        ...
        # Sau khi xử lý lô file hiện tại, gọi lại đệ quy để kiểm tra tiếp
...
def process_video(source_path: str, temp_frame_paths: List[str]) -> None:
    ...
    Thay vì xử lý video trực tiếp, gọi hàm resume_processing_video để đảm bảo:
      ...
      - Khi tất cả frame đã được swap, tạo video đầu ra.
    Sau khi xử lý xong, cập nhật lại danh sách các frame đã được swap vào temp_frame_paths.
    """
    # Nếu chưa có khuôn mặt tham chiếu và không xử lý nhiều khuôn mặt, lấy khuôn mặt tham chiếu từ frame chỉ định
    if not roop.globals.many_faces and not get_face_reference():
        reference_frame = cv2.imread(temp_frame_paths[roop.globals.reference_frame_number])
        reference_face = get_one_face(reference_frame, roop.globals.reference_face_position)
        set_face_reference(reference_face)
    resume_processing_video(source_path, roop.globals.target_path)
    # Sau khi xử lý xong, cập nhật danh sách frame đã swap
