...
def parse_args() -> None:
    ...
    roop.globals.headless = (roop.globals.source_path is not None and 
                              roop.globals.target_path is not None and 
                              roop.globals.output_path is not None)
...
def start() -> None:
    ...
    if temp_frame_paths and hasattr(roop.globals, 'resume_frame') and roop.globals.resume_frame > 1:
        import re
        ...
        update_status(f"Tiếp tục xử lý từ frame số {resume_frame}, tổng số frame cần xử lý: {len(temp_frame_paths)}")
