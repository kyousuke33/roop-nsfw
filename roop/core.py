...
# Đặt biến môi trường: sử dụng single thread giúp cải thiện hiệu suất với CUDA
# – cần thiết lập trước khi import torch
...
def parse_args() -> None:
    ...
    Đã thêm tham số --resume-frame để hỗ trợ tiếp tục xử lý video
    từ một frame cụ thể.
    """
    # Bắt sự kiện Ctrl+C
    ...
    program.add_argument(
        "-s",
        "--source",
        help="Chọn ảnh khuôn mặt",
        dest="source_path",
    )
    program.add_argument(
        "-t",
        "--target",
        help="Chọn ảnh hoặc video đầu vào",
        dest="target_path",
    )
    program.add_argument(
        "-o",
        "--output",
        help="Chọn file hoặc thư mục đầu ra",
        dest="output_path",
    )
    program.add_argument(
        "--frame-processor",
        help=("Các bộ xử lý frame (ví dụ: face_swapper, "
              "face_enhancer, ...)"),
    ...
    program.add_argument(
        "--keep-fps",
        help="Giữ nguyên FPS của video đầu vào",
        dest="keep_fps",
        action="store_true",
    )
    program.add_argument(
        "--keep-frames",
        help="Lưu các frame tạm thời",
        dest="keep_frames",
        action="store_true",
    )
    program.add_argument(
        "--skip-audio",
        help="Bỏ qua âm thanh của video đầu vào",
        dest="skip_audio",
        action="store_true",
    )
    program.add_argument(
        "--many-faces",
        help="Xử lý tất cả các khuôn mặt",
        dest="many_faces",
        action="store_true",
    )
    program.add_argument(
        "--reference-face-position",
        help="Vị trí khuôn mặt tham chiếu",
    ...
    program.add_argument(
        ...
        help="Số thứ tự frame tham chiếu",
    ...
    program.add_argument(
        ...
        help="Khoảng cách khuôn mặt dùng để nhận diện",
    ...
    program.add_argument(
        "--temp-frame-format",
        help="Định dạng ảnh cho frame tạm",
    ...
    program.add_argument(
        "--temp-frame-quality",
        help="Chất lượng ảnh cho frame tạm",
    ...
    program.add_argument(
        "--max-memory",
        help="Giới hạn RAM sử dụng (GB)",
        dest="max_memory",
        type=int,
    )
    program.add_argument(
        "--execution-provider",
        help="Execution provider (ví dụ: cpu, ...)",
    ...
    program.add_argument(
        "--resume-frame",
        help="Số thứ tự frame bắt đầu xử lý (mặc định: 1)",
        dest="resume_frame",
        type=int,
        default=1,
    )
...
def encode_execution_providers(
...
def decode_execution_providers(
    execution_providers: List[str]
) -> List[str]:
    return [
        provider
...
def suggest_execution_providers() -> List[str]:
    return encode_execution_providers(
        onnxruntime.get_available_providers()
    )
...
def pre_check() -> bool:
    if sys.version_info < (3, 9):
        update_status(
            "Phiên bản Python không được hỗ trợ - hãy nâng cấp lên 3.9 hoặc cao hơn."
        )
        return False
...
def start() -> None:
    ...
    Đối với video: trích xuất frame, áp dụng tính năng resume (nếu có),
    xử lý từng frame, ghép lại video và phục hồi âm thanh.
    """
    ...
    if temp_frame_paths and hasattr(roop.globals, "resume_frame") and \
       roop.globals.resume_frame > 1:
        ...
        update_status(
            f"Tiếp tục xử lý từ frame số {resume_frame}, tổng số frame cần "
            f"xử lý: {len(temp_frame_paths)}"
        )
    ...
    if roop.globals.skip_audio:
        ...
        if roop.globals.keep_fps:
            ...
            update_status(
                "Khôi phục âm thanh có thể gây ra một số vấn đề vì FPS không "
                "được giữ nguyên..."
            )
